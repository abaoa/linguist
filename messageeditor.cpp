...
QString MessageEditor::getSourceText()
{
    return m_source->getTranslation();
}

void MessageEditor::setTransText(const QString &text, int index)
{
    if(m_editors.size() > index && index >= 0)
    {
        for(int i = 0; i < m_editors[index].transTexts.size(); i++)
        {
            auto trans = m_editors[index].transTexts[i];
            trans->setTranslation(text,true);
        }
    }
}

void MessageEditor::showTransLogInfo(const QString &text, bool isClear)
{
    if(isClear)
        m_TextBrowser->clear();
