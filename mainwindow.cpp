      ...
      m_statistics(0)
{
    ...
    connect(m_translateDialog, SIGNAL(requestMatchUpdate(bool&)),this, SLOT(updateTranslateHit(bool&)));
    connect(m_translateDialog, SIGNAL(activated(int)),this, SLOT(translate(int)));
    ...
    {
        auto path = recentFiles().lastOpenedFile();
...
bool MainWindow::openFiles(const QStringList &names, bool globalReadWrite)
{
    ...
    modelCountChanged();
    auto files = m_dataModel->srcFileNames();
    recentFiles().addFiles(files);
...
void MainWindow::setupMenuBar()
{
    ...
    connect(m_ui.action_autoTranslation, SIGNAL(triggered(bool)), this, SLOT(autoTranslationSlot(bool)));
...
void MainWindow::setupToolBars()
{
    ...
    QToolBar *release = new QToolBar(this);
    release->setObjectName(QLatin1String("release"));
    release->setWindowTitle(tr("release"));
    this->addToolBar(release);
    m_ui.menuToolbars->addAction(release->toggleViewAction());
...
void MainWindow::transText()
{
    auto text = m_messageEditor->getSourceText();

    auto from = tr("auto");
    auto to = tr("en");
    if(BaiduFanyiPtr->isUsedTsLangage())
    {
        auto sourcelanguage = m_TsFile.mTsFileData.sourcelanguage;
        if(!sourcelanguage.isEmpty())
        {
            auto list = sourcelanguage.split(tr("_"));
            if(list.size() == 2)
            {
                from = list[0];
            }
    ...
    {
        from = BaiduFanyiPtr->fromLangage();
        to = BaiduFanyiPtr->toLangage();
    }

   auto result = BaiduFanyiPtr->trans(text,from,to);
   QEventLoop loop;
   QTimer::singleShot(1000,&loop,SLOT(quit()));
   loop.exec();

   auto rootIndex1 = m_contextView->rootIndex();
   auto rowCount1 = m_sortedContextsModel->rowCount(rootIndex1);

   auto rootIndex2 = m_messageView->rootIndex();
   auto rowCount2 = m_sortedMessagesModel->rowCount(rootIndex2);

   auto row1 = m_contextView->currentIndex().row();
   auto row2 = m_messageView->currentIndex().row();
   bool isStop = ((rowCount1 - 1) == row1 && (rowCount2 - 1 == row2))?true:false;

   if(result.first)
   {
       m_messageEditor->setTransText(result.second,0);
       if(!isStop && m_ui.action_autoTranslation->isChecked())
       {
            m_ui.actionDoneAndNext->trigger();
            transText();
       }else
       {
           m_ui.actionDone->trigger();
       }
   }else
   {
       m_messageEditor->showTransLogInfo(result.second);
       if(!isStop && m_ui.action_autoTranslation->isChecked())
       {
            m_messageEditor->setTransText(text,0);
            m_ui.actionNext->trigger();
            transText();
       }else
       {
           m_ui.action_autoTranslation->trigger();
       }
   }

}
void MainWindow::autoTranslationSlot(bool checked)
{
    if(checked)
    {
...
void MainWindow::on_action_autoTranslationSetting_triggered()
{
    auto dialog = new BaiduFanyiSettingDialog(this);
    dialog->exec();
}
